    ...
    submitSuccess: function ($form, event) {
      ...
      if (!validation) {
        ...
        $("#success > .alert-success")
          ...
          .append("</button>");
        $("#success > .alert-success").append("<strong>Success Login</strong>");
        $("#success > .alert-success").append("</div>");
        //clear all fields
        $("#loginForm").trigger("reset");
        localStorage.setItem("currentUser", JSON.stringify(validation));
        swal("Success", "Login Successfully!", "success").then((response) => {
          if (response) {
            location.href = "../screen/dashboard.html";
          } else {
